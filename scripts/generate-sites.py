...
try:
    ...
    TEMPLATE_COMBINER_AVAILABLE = True
except ImportError:
    TEMPLATE_COMBINER_AVAILABLE = False
...
USE_MODULAR_SYSTEM = True  # Usar sistema modular de paletas+fuentes+layouts
...
def main():
  ...
  Modo no-interactivo:
    ...
    # Generar templates CSS si está habilitado el sistema modular
    templates_metadata = None
    if USE_MODULAR_SYSTEM and TEMPLATE_COMBINER_AVAILABLE:
        print(f"\n🎨 Generando {cantidad} templates CSS modulares...")
        print("=" * 60)
        combiner = TemplateCombiner()
        
        # Mostrar estadísticas
        stats = combiner.calcular_posibilidades()
        print(f"   📊 {stats['palettes']} paletas × {stats['fonts']} fuentes × {stats['layouts']} layouts")
        print(f"   💫 {stats['total_combinations']:,} combinaciones únicas posibles\n")
        
        # Generar templates
        templates_metadata = combiner.generar_multiples_templates(
            num_templates=cantidad,
            output_dir=CSS_DIR,
            aleatorio=True
        )
        print(f"\n✅ Templates CSS generados en '{CSS_DIR}/'\n")
    else:
        if USE_MODULAR_SYSTEM:
            print("⚠️  Sistema modular no disponible, usando templates existentes\n")
    ...
    for i in range(1, cantidad + 1):
        ...
        if sites_metadata and i <= len(sites_metadata):
            ...
            template_info = ""
            if templates_metadata and i <= len(templates_metadata):
                tmpl = templates_metadata[i - 1]
                template_info = f" [P:{tmpl['palette'][:15]} F:{tmpl['font'][:15]} L:{tmpl['layout'][:15]}]"
            print(f"  [{i}/{cantidad}] 📦 {metadata['nombre']} ({metadata['dominio']}){template_info}")
        else:
    ...
    if templates_metadata:
        print(f"🎨 {len(templates_metadata)} templates CSS únicos generados")
        if USE_MODULAR_SYSTEM and TEMPLATE_COMBINER_AVAILABLE:
            stats = combiner.calcular_posibilidades()
            print(f"   💫 {stats['total_combinations']:,} combinaciones posibles en total")
    print()
