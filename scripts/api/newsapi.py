...
from utils.utils import enrich_with_full_text, save_articles, print_summary
...
def fetch_newsapi(query: str = 'tecnología', 
                  language: str = 'es',
                  page_size: int = 20,
                  enrich: bool = True,
    ...
    try:
        ...
        if data.get('status') != 'ok':
            raise Exception(f"Error en respuesta API: {data.get('message', 'Unknown error')}")
...
def main():
    ...
    parser.add_argument('--query', type=str, default='tecnología',
                       help='Términos de búsqueda (default: "tecnología")')
    ...
    parser.add_argument('--no-enrich', action='store_true',
                       help='No extraer texto completo')
