...
class NewsAutomationSystem:
    ...
    def step1_download_news(self) -> list:
        """
        Paso 1: Descargar noticias de tecnología usando la API seleccionada
        
        Returns:
            Lista de artículos descargados
        """
        print(f"\n{'='*70}")
        print(f"📥 PASO 1: Descargando noticias de tecnología (API: {self.api_source.upper()})")
        print(f"{'='*70}")
...
║  🤖 SISTEMA AUTOMATIZADO DE NOTICIAS CON IA                      ║
║                                                                   ║
║  • Descarga automática de noticias de tecnología                 ║
║  • {mode:<58} ║
