#!/usr/bin/env python3
"""
Template Combiner - Sistema de Combinación Modular
Combina paletas de colores, fuentes y layouts para generar templates CSS únicos
"""
...
class TemplateCombiner:
    """Combina módulos CSS independientes en templates únicos"""
    ...
    def generar_combinacion_unica(self, 
        ...
        """
        Genera una combinación única de palette + font + layout
        
        Args:
            palette_index: Índice de paleta (None = aleatorio)
            font_index: Índice de fuente (None = aleatorio)
            layout_index: Índice de layout (None = aleatorio)
            
        Returns:
            dict: Combinación completa
        """
        # Si no se especifican índices, elegir aleatoriamente
    ...
    def generar_css_combinado(self, combinacion: Dict[str, any]) -> str:
        """
        Genera CSS completo combinando todos los módulos
...
/* Responsive footer columns using auto-fit */
.footer-grid.cols-2 {
...
"""
    ...
    def generar_template_css(self, 
        ...
        Args:
            template_number: Número del template
    ...
    def generar_multiples_templates(self, 
        ...
        """
        Genera múltiples templates únicos
        ...
        print(f"   {total_palettes} paletas × {total_fonts} fuentes × {total_layouts} layouts\n")
        
        if aleatorio:
            print(f"🎲 Generando {num_templates} templates aleatorios...\n")
            for i in range(1, num_templates + 1):
                metadata = self.generar_template_css(i, output_dir)
                metadatos.append(metadata)
                print(f"✅ Template {i}: {metadata['palette']} + {metadata['font']} + {metadata['layout']}")
        else:
            print(f"🔢 Generando {num_templates} templates secuenciales...\n")
            combo_idx = 0
            for i in range(1, num_templates + 1):
                # Calcular índices secuenciales
    ...
    def calcular_posibilidades(self) -> Dict[str, int]:
        ...
        Returns:
            dict: Información sobre posibilidades
        """
...
def main():
    """Función principal"""
    print("🎨 Template Combiner - Sistema Modular CSS")
    print("=" * 70)
    ...
    print(f"   ───────────────────────────────")
    print(f"   Total combinaciones únicas: {stats['total_combinations']:,}")
    ...
    print(f"\n💾 ¿Generar {num_templates} templates en {output_dir}? (s/n): ", end="")
    ...
    if response == 's':
        ...
        print(f"\n✅ Se generaron {len(metadatos)} templates")
